...
import { Home, LogOut, UserCog, MessageSquare, Menu } from 'lucide-react';
...
export function AuthenticatedPageLayout({ children, title = "Empathy.AI" }: AuthenticatedPageLayoutProps) {
  const { user } = useAuth();
      ...
      <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-16 max-w-screen-2xl items-center justify-between">
          <Link href="/chat" className="flex items-center space-x-2">
            {/* Using an inline SVG for the logo as per guidelines (no image files) */}
